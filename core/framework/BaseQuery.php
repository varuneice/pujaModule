...
abstract class BaseQuery implements IteratorAggregate {
    ...
    public function getIterator(): \Traversable {
        $result = $this->execute();
        // execute() can return false on failure; wrap in an empty ArrayIterator to satisfy Traversable
        return ($result instanceof \Traversable) ? $result : new \ArrayIterator([]);
    }
    ...
    private function clauseNotEmpty($clause) {
        if ($this->clauses[$clause]) {
            $stmt = $this->statements[$clause];
            // PHP 8: count() throws TypeError on non-countable; handle string/null statements
            if (is_array($stmt)) {
                return (boolean) count($stmt);
            }
