       ...
       if ($("#dialogDelete").length > 0) {
                ...
                buttons: [{
                        ...
                        click: function() {
                            $.ajax({
                                ...
                                data: {
                                    id: $('#record_id').text(),
                                    controller: 'pujaregistration',
                                action: 'deleteImage'
                                },
                                url: url + "index.php?controller=pujaregistration&action=deleteImage",
                                beforeSend: function() {
        ...
        $(document).delegate('#reset-btn-id', 'click', function (e) {
            ...
            if (val == 'stripe') {
                ...
                var form = document.getElementById('paidparking-frm-id');
            ...
            e.stopImmediatePropagation();
            debugger;
            var proxyamount = parseInt($("#proxyprice").val());
            var totalamount = parseInt($("#total_value4").val());
            if(totalamount != proxyamount){
            ...
            var parkingprice = totalprice.replace(/ /gi, '').trim();
            var totalparkingprice =  parkingprice.split(".");
            var tot = totalparkingprice[0];
            if (cmCode != null) {
                        ...
                        click: function () {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    id: $('#record_id').text(),
                                    cat: cat,
                                    controller: 'parkingadmin',
                                    action: 'delete'
                                },
                                url: url + "index.php?controller=parkingadmin&action=delete",
                                success: function (res) {
