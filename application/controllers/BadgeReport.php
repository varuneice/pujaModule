...
class BadgeReport extends App {
  ...
  function __construct() { 
    ...
    function GetBadgeDetail(){
        GzObject::loadFiles('Model', array('BadgeReport'));
        $BadgeReportModel = new BadgeReportModel();
        $arr = $BadgeReportModel->GetBadgeDetails();
        $response = array();
        if ($arr == null) {
            $response['status'] = 300;
            $response['data'] = array();
                ...
                $date = DateTime::createFromFormat('Y-m-d', $arr[0]['Date']);
                if ($date) {
                    $month = strtoupper(substr($date->format('F'), 0, 3)); 
                    $formattedDate = $month . '-' . $date->format('d') . '-' . $date->format('Y'); 
                    $arr[0]['Date'] = $formattedDate;
                }
            }
            $response['status'] = 200;
            $response['data'] = $arr;
