...
class parkingadmin extends App
{
    var $option_arr = null;
    var $layout = 'admin';
    function beforeFilter() {
        ...
        if (!$this->isLoged() && ($_REQUEST['action'] ?? '') != 'login') {
            ...
            if (($_REQUEST['action'] ?? '') != 'edit' && ($_REQUEST['action'] ?? '') != 'payment') {
                $_SESSION['err'] = 2;
    ...
    function index()
    { 
        GzObject::loadFiles('Model', array('paidparking'));
        $paidparkingModel = new paidparkingModel();
        $opts = array();
        $arr = $paidparkingModel->getAlldata($opts);
        $this->tpl['arr'] = $arr;
    ...
    function edit()
    {
        GzObject::loadFiles('Model', array('paidparking'));
        $paidparkingModel = new paidparkingModel();
        if (!empty($_POST['edit_parkingregistrationdata'])) {

            $data = array();
             if(($_POST['seniorseventyplus'] ?? '') == "" || ($_POST['seniorseventyplus'] ?? '') == null){
                 $_POST['seniorseventyplus'] = '';
             }
            $id = $_POST['id'] ?? '';
            $status = $_POST['status'] ?? '';
            if ($status == 'Active') {
                $url = INSTALL_URL . "parkingadmin/payment/$id";
                //$url = "http://localhost:8082/HDBS_Payment/Parking&Badges/parkingadmin/payment/$id";
               
                $fullname = ($_POST['First_name'] ?? '') . ' ' . ($_POST['Last_name'] ?? '');
                $parkingfield = $_POST['parkingfield'] ?? '';
                ...
                <tbody>
                <tr>
                <td style="text-align: center; border: 1px solid black; margin-left: auto; border-collapse: collapse; margin-right: auto;"><strong>Paid Parking Registration Payment Request</strong></td>
                </tr>
                </tbody>
                </table>
                </div>
                ...
                <td style="border: 1px solid black; margin-left: auto; border-collapse: collapse; margin-right: auto; text-align: left;">' . $parkingfield . '</td>
                </tr>
                <tr>
                ...
                <tr>
                <td colspan =2 style="border: 1px solid black; margin-left: auto; border-collapse: collapse; margin-right: auto; text-align: center;">Please make your payment to complete your Paid Parking registration &nbsp;&nbsp;</td>
                </tr>
               </tbody>
                </table>
                </div>
                ...
                if ($mobileno != null) {
                    $msg = 'Houston Durga Bari: ' .$fullname. ' your Paid parking Request for '.$parkingfield. '  Houston Durga Bari has been approved. Please check your email and complete final payment. ';
            ...
            if ($status == 'cancelled') {
                ...
                $message ='<html><head> <style></style></head>
                    ...
                    <div style="text-align: justify;">
                        <div style="text-align: center;">
                            <table style="height: 77px; margin: 0 auto; border-collapse: collapse;">
                                <tbody>
                                    <tr>
                                        <td style="text-align: center;">
                                            <img src="' . INSTALL_URL . 'application/web/upload/image/create.png" alt="" width="396" height="66" />
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div style="padding: 2px 272px 4px 297px;text-align:initial;">
                            <p style="style="margin-left: 22em;">'.$name.'</p>
                            <p style="style="margin-left: 22em;">'.$cancelremarks.'</p>
                            <p style="color:black;">Regards, <br>Houston Durga Bari Society</p>
                        </div>
                    </div>
                </body>
                    </html>
               ...
               $email = $_POST['email'] ?? '';
                ...
                $mobileno= $_POST['alternatenumber'] ?? '';
                if ($mobileno != null) {
                    $msg = 'Houston Durga Bari: ' .$fullname. ' your Paid parking Request for '.$parkingfield. '  Houston Durga Bari has been Cancelled the request. ';
                   $this->SendSMS($mobileno, $msg);
                }
            }
            
            if($status == 'confirmed'){
                if(($_POST['PaymentOption'] ?? '') =="Credit Card"){
                    $_POST['PaymentOption'] = 'stripe';
                }
                else if(($_POST['PaymentOption'] ?? '') =="Zelle"){
                    $_POST['PaymentOption'] = 'others';
                }
                else if(($_POST['PaymentOption'] ?? '') =="Cash"){
                    $_POST['PaymentOption'] = 'cash';
                }
                else if(($_POST['PaymentOption'] ?? '') =="Check"){
                    $_POST['PaymentOption'] = 'check';
                }
                else if(($_POST['PaymentOption'] ?? '') =="Direct Deposit"){
                    $_POST['PaymentOption'] = 'directdeposit';
                }
            }

            $paidparkingModel->update($_POST);
            ...
            if (!$this->isAdmin()) {
                ...
                Util::redirect(INSTALL_URL . "parkingadmin/index");
            }
        }
        $id = $_GET['id'] ?? '';
        $parkingarr = $paidparkingModel->get($id);
        $this->tpl['parkingarr'] = $parkingarr;
    ...
    function payment(){
        GzObject::loadFiles('Model', array('paidparking' ,'ConfirmCode', 'Donation', 'idnumbers'));
        $paidparkingModel = new paidparkingModel();
            ...
            if (($_POST['PaymentOption'] ?? '') == 'others') {
                ...
                if ($oid != null) {
                    ...
                    $DonationModel->SaveDataInDonation($value);
                       ...
                       $email = $_POST['email'] ?? '';
                        ...
                        $mobileno = $data['tele1'];
                        if ($data['tele1'] != null) {
                            $msg = 'Houston Durga Bari: Paid Parking Confirmation are Member Id: ' . $memberid . ', Order Id: ' . $oid . ', Member Name: ' . $fullname . ', Parking Spot: ' . $parkingfield . ',  Amount: $' . $amount . ' , Payment Method: ' . $paymentmethod.' , Pay Date: ' . $payfinaldate;
                            $this->SendSMS($mobileno, $msg);
                        }
                        $this->tpl['arr'] = $paidparkingModel->get($_POST['id'] ?? '');
                    //exit();
                }
                ...
                try {
                    ...
                    if ($payment->status == 'succeeded') {
                        ...
                        $this->tpl['arr'] = $paidparkingModel->get($id);
                    } else {
        ...
        $id = $_GET['id'] ?? '';
        if ($id != null || $id = "") {
            $paidregistrationarr = $paidparkingModel->get($id);
            $this->tpl['paidregistrationarr'] = $paidregistrationarr;
        }
    ...
    function delete()
    {
        ...
        if($cat == 1){
            $paidparkingModel->deleteFrom($paidparkingModel->getTable())
             ->where('id', $id)->execute();
        }
        $opts = array();
        Util::redirect(INSTALL_URL . "parkingadmin/index");
    ...
    public function export()
    {
        ...
        $opts = array();
        $data = $paidparkingModel->getAlldata($opts);

        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename=Paid_Parking_export.csv');
