...
class upload {
    ...
    /**
     * Language used for translated messages
     *
     * @access public
     * @var string
     */
    ...
    /**
     * Raw upload_max_filesize value from php.ini
     *
     * @access public
     * @var string
     */
    ...
    function init() {
        ...
        $this->dir_chmod                = 0755;     // owner rwx, group/others rx
    ...
    function getsize($size) {
        ...
        $numericSize = is_numeric($size) ? (float) $size : (float) substr($size, 0, -1);
        switch($last) {
            case 'g':
                $numericSize *= 1024;
            case 'm':
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                // utf8_encode/utf8_decode removed in PHP 9.0 — use mb_convert_encoding instead
                $accentedChars    = 'ŠŽšžŸÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝàáâãäåçèéêëìíîïñòóôõöøùúûüýÿ';
                $replacementChars = 'SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy';
                $bodyLatin1 = mb_convert_encoding($this->file_dst_name_body, 'ISO-8859-1', 'UTF-8');
                $accentedLatin1 = mb_convert_encoding($accentedChars, 'ISO-8859-1', 'UTF-8');
                $transliterated = strtr($bodyLatin1, $accentedLatin1, $replacementChars);
                $this->file_dst_name_body = mb_convert_encoding($transliterated, 'UTF-8', 'ISO-8859-1');
