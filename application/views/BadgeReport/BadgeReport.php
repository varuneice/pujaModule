...
<div class="container" style="position: relative;padding: 40px 0px 60px;">
    <div class="row">
        ...
        <div id="tbltblBadgeRecordStatus" class="pt-3" style="display:none;">
            <table width="100%" class="table table-hover" id="">
                <thead>
                    <tr>
                        ...
                        <th>Spouse Name</th>
                        <th>Seq No</th>
                        <th>Additional Magazine</th>
                        <th>Date of Issue</th>
                        <th>Badge Status</th>
                    </tr>
                </thead>
                <tbody id="tblBadgeRecord">
...
<script>
    $(function () {
        $("#term").autocomplete({
            //source: "http://localhost:8080/HDBS_Payment/pujaModule/ajax-db-search.php",
    ...
    function getBadgeRecord(e) {
        const memberId = $("#termMember").val();
        ...
        $.ajax({
            ...
            url: url1 + "load.php?controller=BadgeReport&action=GetBadgeDetail",
            success: function (res) {
              
                let response = res.match(/{.*}/s);
                var data = response[0] ? JSON.parse(response[0]) : [];
                if (data.status === 200) {
    ...
    function setBadgeStatus(obj) {
        
        let output = "";
        for (i = 0; i < obj.length; i++) {

            const memberId = obj[i].MID != null ? obj[i].MID : '';
            const firstName = obj[i].F_Name != null ? obj[i].F_Name : '';
            const lastName = obj[i].L_Name != null ? obj[i].L_Name : '';
            const spouseName = obj[i].Sp_FName != null ? obj[i].Sp_FName : '';
            const seqNo = obj[i].SeqNo != null ? obj[i].SeqNo : '';
            const magazine = obj[i].Magazines != null ? obj[i].Magazines : '';
            const badgeStatus = obj[i].Status != null ? obj[i].Status : '';
            const badgeIssueDate = obj[i].Date != null ? obj[i].Date : '';

            output += `<tr>
            <td style="white-space:normal;">${memberId}</td>
            <td style="white-space:normal;">${firstName + " " + lastName}</td>
            <td style="white-space:normal;">${spouseName}</td>
            <td style="white-space:normal;">${seqNo}</td>
            <td style="white-space:normal;">${magazine}</td>
            <td style="white-space:normal;">${badgeIssueDate}</td>
            <td style="white-space:normal;">${badgeStatus}</td>
        </tr>`;
        }
        if (output) {
            $("#tblBadgeRecord").html("");
            $("#tblBadgeRecord").html(output);
            $("#tbltblBadgeRecordStatus").show();
        }
        else {
            $("#tblBadgeRecord").html("");
            $("#tbltblBadgeRecordStatus").hide();
  ...
  $("#btnReset").click(function() {
    ...
    //Method for reset the search value 
      function dataReset(){
        $("#termMember").val("");
         $("#term").val("");
      }
